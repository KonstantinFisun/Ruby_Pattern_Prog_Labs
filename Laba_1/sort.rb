...
«гласная-согласная» и «согласная-гласная» в строке.
    6. Отсортировать в порядке квадратичного отклонения дисперсии максимального
среднего веса ASCII-кода тройки символов в строке от максимального среднего
веса ASCII-кода тройки символов в первой строке.
    ")
    sel = gets.chomp
    select_metod(sel)\
end
...
«гласная-согласная» и «согласная-гласная» в строке - #{metod_5(init_list_str)}")
    when "6"
      puts("В порядке квадратичного отклонения дисперсии максимального
среднего веса ASCII-кода тройки символов в строке от максимального
среднего веса ASCII-кода тройки символов в первой строке - #{metod_6(init_list_str)}")
    else
...
def metod_5(list_str)
  list_str.sort do
    |a,b| (count_vowel_consonant(a)-count_consonant_vowel(a)) <=> (count_vowel_consonant(b)-count_consonant_vowel(b))
  end
end
...
def metod_6(list_str)
  list_str.sort do
    |a,b| (max_ave_wei_of_3_char(a)-max_ave_wei_of_3_char(list_str[0])) <=> (max_ave_wei_of_3_char(b)-max_ave_wei_of_3_char(list_str[0]))
  end
end

# Находим максимальный средний вес тройки символов в строке
def max_ave_wei_of_3_char(str)
  #(1..10).each_slice(3) {|a| p a}
  str.scan(/.{3}/).collect {|a| a.each_byte.inject{|b,c| b+c}/3}.max
